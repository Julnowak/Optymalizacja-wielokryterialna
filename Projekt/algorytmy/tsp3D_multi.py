...
def plot_graphs_static(all_paths, terrain, starts, ends):
    ...
    for i, path in enumerate(all_paths):
        # W path mamy [(r,c), (r2,c2), ...], do plotu dajemy odwrotnie (c,r)
        X = [p[1] for p in path]
        Y = [p[0] for p in path]
...
def plot_graphs_animation(all_paths, terrain, starts, ends):
    ...
    def update(frame):
        # wyczyść rysunek poza warstwą tła
        ax.clear()
        ax.imshow(terrain, origin='upper', cmap="magma")
        ...
        # (jeśli jeszcze istnieje taki krok w jego ścieżce)
        for i, path in enumerate(all_paths):
            c = colors[i % len(colors)]
            # narysuj wszystkie poprzednie kroki do 'frame', aby widać było trajektorię
            # można też narysować tylko bieżącą pozycję
...
class GeneticTSP3DPath:
    ...
    - Funkcja celu: dystans 3D + kara kolizji.
    ...
    def __init__(self, terrain, occupied_paths=None):
        ...
        # Buforowanie dystansów (p1, p2) -> dystans 3D
        self.distance_cache = {}
    ...
    def distance_path(self, path):
        """
        Suma dystansów 3D kolejnych punktów + kara za kolizje.
        """
        key = tuple(path)
    ...
    def valid_path(self, path):
        ...
        for i in range(len(path) - 1):
            r1, c1 = path[i]
            r2, c2 = path[i + 1]
            # sprawdzamy, czy (r2-r1, c2-c1) jest w zbiorze dozwolonych ruchów
            dr = r2 - r1
            dc = c2 - c1
    ...
    def generate_random_path(self, start, end):
        """
        Tworzymy ścieżkę z preferencją do celu, ale dopuszczamy ruch w 8 kierunkach.
        max ~2*(Nx+Ny) kroków (lub do skutku).
        """
...
def run_multi_robot_tsp3d_path(
    ...
    for i, (st, en) in enumerate(zip(starts_2d, ends_2d)):
        ...
        # żeby kolejne roboty unikały kolizji.
...
if __name__ == "__main__":
    ...
    # Parametry GA (możesz zmienić wg potrzeb)
    pop_size = 50
